...
const PokemonForm: FunctionComponent<Props> = ({ pokemon, isEditForm }) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isFormValid) {
      ...
      //si formulaire édition -> update, sinon add
  ...
  const validateForm = () => {
    ...
    if (isAddForm()) {
      ...
      //vérifie que l'url saisie par l'utilisateur commence bien par la string "start"
      //pour vérifier que les images proviennent toutes du même site
      if (
                    ...
                    <div key={type} style={{ marginBottom: '10px' }}>
                      <label>
                        <input
                          ...
                          onChange={(event) => selectType(type, event)}
                        ></input>
                        <span>
